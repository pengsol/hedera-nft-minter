...
export default function ConnectToDiscordButton({
  ...
  const connectToDiscordButtonOnClick = useCallback(() => {
    if (!isEnabled) {
      return
    }
  ...
  const connectToDiscordButtonImage = useMemo(() => (
    logoImageSrc ? (
  ...
  const connectToDiscordButtonContent = useMemo(() => {
  ...
  const connectToDiscordButtonClassNames = useMemo(() => (
  ...
  return (
    <button
      disabled={!isEnabled}
      className={connectToDiscordButtonClassNames}
      onClick={connectToDiscordButtonOnClick}
      type='button'
    >
      {connectToDiscordButtonImage}
      <p>{connectToDiscordButtonContent}</p>
    </button>
  )
}
